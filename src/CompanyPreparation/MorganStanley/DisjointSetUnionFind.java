...
public class DisjointSetUnionFind {
    ...
    public static int findParentIndex(int[] arr, int x){
        ...
        //directly returning arr[x] should also work, as arr has direct parents
        return findParentIndex(arr,arr[x]);
    }

    public static boolean ifCycleFormed(int[] arr, int x, int y){
    ...
    public static void union(int[] arr, int x, int y){
        if(ifCycleFormed(arr,x,y)){
            System.out.println("cycle is formed... "+x+" & "+y+" belong to same set");
            return;
        }
